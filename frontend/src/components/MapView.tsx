...
function MapView({ activeLayer, simulationData }: MapViewProps) {
              ...
              .setHTML(`
                <div class="p-2">
                  <h3 class="font-bold text-lg mb-2">${
                    properties?.station || "AQI Station"
                  }</h3>
                  <p class="text-sm"><strong>AQI:</strong> ${
                    properties?.aqi || "N/A"
                  } (${properties?.category || "Unknown"})</p>
                  <p class="text-sm"><strong>PM2.5:</strong> ${
                    properties?.pm25 || "N/A"
                  } µg/m³</p>
                  ${
                    properties?.pm10
  ...
  useEffect(() => {
    if (!map.current || !map.current.isStyleLoaded() || !simulationData || !isMountedRef.current) {
      return;
    }
    ...
    try {
      ...
      if (source && simulationData.after) {
        if (
          simulationData.after.type === "FeatureCollection" &&
          Array.isArray(simulationData.after.features)
        ) {
          console.log(
            "[MapView] Applying simulation AFTER data, features=",
            simulationData.after.features.length
          );
  ...
  return (
    <div className="relative w-full h-full overflow-hidden">
      ...
      {error && (
        <div className="absolute top-4 left-1/2 -translate-x-1/2 z-50 bg-red-500 text-white px-4 py-2 rounded shadow-lg">
          {error}
        </div>
      )}
      <div
