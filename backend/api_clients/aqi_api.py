...
Requirements:
- Use WAQI /here endpoint:
  https://api.waqi.info/feed/here/?token=TOKEN
- Use WAQI_TOKEN from environment, no hard-coded token.
- Fallback: generate realistic synthetic AQI points around Singapore.
"""
import os
import json
import requests
...
WAQI_URL = "https://api.waqi.info/feed/here/"
...
def _create_default_aqi_geojson() -> Dict[str, Any]:
    """Create default valid GeoJSON for Singapore AQI with multiple points."""
    return {
        ...
        "features": [
            ...
            {
                "type": "Feature",
                "geometry": {"type": "Point", "coordinates": [103.8450, 1.3000]},
                "properties": {
                    "aqi": 55,
                    "pm25": 20.0,
                    "pm10": 28.0,
            ...
            {
                "type": "Feature",
                "geometry": {"type": "Point", "coordinates": [103.8600, 1.2800]},
                "properties": {
                    "aqi": 75,
                    "pm25": 30.0,
                    "pm10": 42.0,
...
def _convert_waqi_to_geojson(waqi_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if "data" in waqi_data and "iaqi" in waqi_data["data"]:
        ...
        city = data.get("city", {})
        coords = city.get("geo", [1.2966, 103.8198])
...
def fetch_live_aqi() -> Dict[str, Any]:
    ...
    Uses WAQI /here with WAQI_TOKEN from environment.
    Fallback: data.gov.sg PM2.5, then synthetic mock AQI points.
    """
    token = os.getenv("WAQI_TOKEN")

    if not token:
        logger.warning("WAQI_TOKEN not set; using synthetic AQI data")
    ...
    try:
        ...
        if waqi_data.get("status") == "ok":
            ...
            if geojson_data.get("features"):
                logger.info(
                    f"WAQI live AQI returning {len(geojson_data.get('features', []))} feature(s)"
                )
                return geojson_data
        ...
        logger.warning("WAQI/data.gov.sg returned no data, using synthetic AQI")
        return _load_mock_data()
